...
export default function Header5() {
  ...
  const productItems = [
    { title: "Analizler", desc: "Gerçek zamanlı veri takibi.", icon: "📊" },
    { title: "Otomasyon", desc: "İş akışlarınızı hızlandırın.", icon: "⚡" },
    { title: "Raporlama", desc: "Detaylı PDF raporlar.", icon: "📑" },
    { title: "Entegrasyon", desc: "Tüm araçlarınızla uyumlu.", icon: "🔌" },
  ];
             ...
             >
                <button 
                  className={cn(
                    ...
                    isScrolled ? "text-gray-600 hover:text-black" : "text-gray-700 hover:text-[#0073FF]"
                  )}
                >
                ...
                >
                   ...
                   <div className="grid gap-2">
                      {productItems.map((item, i) => (
                          <a key={i} href="#" className="flex items-start gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors group/item">
                             <div className="bg-gray-100 p-2 rounded-md text-lg group-hover/item:bg-blue-50 group-hover/item:scale-110 transition-all">{item.icon}</div>
                             <div>
                                <div className="font-bold text-gray-800 text-sm group-hover/item:text-blue-600 transition-colors">{item.title}</div>
                ...
                {/* Animated Underline */}
                <div className="absolute bottom-0 left-0 w-0 h-0.5 bg-[#0073FF] transition-all duration-300 group-hover:w-full"></div>
             </div>
           ))}
        </nav>
