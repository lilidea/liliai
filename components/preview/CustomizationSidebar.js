...
export default function CustomizationSidebar({ isOpen, onClose, activeSection, activeCategory }) {
    ...
    const handleSend = async (text = input) => {
        ...
        try {
            ...
            // 🚀 Apply AI-driven state changes
            if (data.success !== false) {
                if (data.updates) {
                    updateSiteData(data.updates);
                }
                if (data.selections) {
                    Object.entries(data.selections).forEach(([cat, id]) => {
                        updateSelection(cat, id);
                    });
                }
            }
            ...
            setMessages(prev => [...prev, {
                ...
                applied: !!(data.updates || data.selections)
            }]);
