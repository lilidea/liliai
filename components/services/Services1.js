...
const Services1 = () => {
   ...
   const defaultServices = [
      { title: "Kaliteli Hizmet", icon: <Globe size={24} />, desc: "Sektördeki tecrübemizle size en kaliteli hizmeti sunuyoruz." },
      { title: "Profesyonel Ekip", icon: <Layers size={24} />, desc: "Uzman kadromuzla tüm ihtiyaçlarınıza çözüm üretiyoruz." },
      { title: "Müşteri Memnuniyeti", icon: <Shield size={24} />, desc: "Sizin memnuniyetiniz bizim için her zaman önceliklidir." },
   ];

   const rawItems = content.items || defaultServices;

   const services = rawItems.map(item => ({
      ...item,
      icon: typeof item.icon === 'string' ? <Box size={24} /> : (item.icon || <Box size={24} />)
   }));
